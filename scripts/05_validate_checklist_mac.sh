...
RESULTS="${1:-results}"
THREADS="${2:-?}"   # recebe THREADS do pipeline como segundo argumento
...
if [ -f "$TAX_TSV" ]; then
    ...
    TOTAL=$(awk 'NR>1' "$TAX_TSV" | wc -l | tr -d ' ')
...
NPROC=$(sysctl -n hw.logicalcpu 2>/dev/null || echo "?")
pass "CPUs disponíveis: ${NPROC}"
echo "  → Threads configuradas no conf: ${THREADS}"
