#!/usr/bin/env bash
# =============================================================================
# run_pipeline_mac.sh — Pipeline 16S rRNA (QIIME 2 direto, sem Nextflow) — macOS
#
# Uso:
...
# ── Compatibilidade macOS: conda activate usa variáveis não definidas ────────
# activate-gfortran_osx-64.sh referencia $GFORTRAN sem inicializar, causando
# erro com set -u. Desativamos -u apenas durante o conda activate.
_conda_activate() { set +u; conda activate "$1"; set -u; }
...
# ── PASSO 2: Manifest ─────────────────────────────────────────────────────────
_build_manifest() {
    _conda_activate qiime2-amplicon-2026.4
...
hdr "╚══════════════════════════════════════════════════════════╝"
echo ""
bash "$(dirname "$0")/../scripts/05_validate_checklist_mac.sh" "${OUTDIR}" "${THREADS}"
